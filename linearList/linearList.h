...
template<typename ElemType>
void SeqList<ElemType>::DeleteSame() {
    int MovePtr = 1, StayPtr = 1;
    int L = this->length;
    for (MovePtr; MovePtr < L; MovePtr++)
    {
        if (this->elems[MovePtr] == this->elems[StayPtr]) continue;
        else {
            this->elems[StayPtr] = this->elems[MovePtr];
            StayPtr++;
            this->length = this->length - (MovePtr - StayPtr);
        }
    }
    length--;
}
